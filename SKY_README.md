...
```text
OpenCode release : v1.18.32
Local branch     : sky/v1.18.32
Required Bun     : 1.3.14
```
...
```bash
git merge-base --is-ancestor v1.18.32 HEAD
```
...
```text
1.18.32-sky.abcdef1234
```

含意：

```text
1.18.32       upstream stable base
...
Bun version **不 hard-code 在 helper**。目前 checkout 若是 `v1.18.32`，helper 會從 `package.json` 取得 `bun@1.3.14`。
...
## 9. 觀察 upstream 版本與 Release 候選

日常只要檢查目前 adopted release 與 upstream 是否出現新的 strict version tag 時，使用：
...
└── upstream fetch 後 strict vX.Y.Z tags 中版本最高者
```

範例輸出（版本號只代表當時的 repository state）：

```text
Latest adopted         : v1.18.32
Highest observed tag   : v1.19.0
Newer tag observed     : YES
Release review required: YES
```

Git refs 只能證明 upstream 存在某個 tag，**不能單獨證明該 tag 已發布成 stable release**。GitHub Release 的 draft / prerelease / published state 不存在於 Git tag ref 中。因此 strict `vX.Y.Z` tag 只能當成 release review signal，不能直接當成 `Latest available release` 或 upgrade target。

`Release review required: YES` 只表示 upstream 出現較新的 strict version tag；在把它視為 available stable release 前，至少要另外確認 upstream release metadata / announcement，並確認 target tag 與該 published stable release 對應。對 GitHub-hosted upstream，可使用 GitHub Releases 頁面或 API 作為這層證據。

這個工具不會建立 branch、migration patches、build 或 publish；`Latest adopted` 也不代表目前 checkout 的 branch。

即使確認 target 是 published stable release，也不代表：
...
對 release maintenance 的 Git-side observation refs 是：
...
看 strict version-tag candidates：
...
`git fetch upstream` 可能讓 local strict official tags 比 `origin` 的 version-tag archive 更新；這是正常的 repository state，直到明確執行 archive publication 為止。

`sky-tools/check-release.sh` 只負責 adopted / version-tag observation，不會把 tags push 到 `origin`，也不宣稱 tag 等同於 published stable release。tag authority / archive boundary、strict tag filtering、identity audit 與 publication procedure 以：
...
只有在 Section 9 的 tag observation 之外，已另外確認 upstream 的 published stable release metadata / announcement 後，才把對應 tag 視為 **available release**。新的 available stable release 不是自動 upgrade event。
...
```text
currently adopted : v1.18.32
available upstream: v1.18.33, v1.18.34, v1.19.0
chosen release     : v1.19.0
```
...
```bash
git fetch upstream
git show-ref --verify refs/tags/v1.19.0
```
...
```bash
git switch --no-track -c sky/v1.19.0 v1.19.0
```
...
```bash
git show v1.19.0:package.json | cat
...
```text
local(maintenance): migrate release maintenance workflow to v1.19.0
```
...
對目前 `sky/v1.18.32`，local patch 或 maintenance change 完成後至少：
...
這組 command 是 **目前 v1.18.32 經 target upstream contract review 確認的 validation baseline**，不是所有 future release 的永久 contract；migration validation 必須實際通過後才能 publish。
...
`sky/v1.18.32` 的 baseline maintenance infrastructure commit subject：

```text
local(maintenance): migrate release maintenance workflow to v1.18.32
```
...
新的 official `vX.Y.Z` tag 只代表新的 version tag 被觀察到；Git tag 本身不能證明它已發布成 stable release。先依 Section 9 另外驗證 upstream release metadata / announcement，再決定它是否是 available stable release。

即使已確認是 available stable release，除非 Sky 明確決定採用，否則不要：
...
快速檢查 adopted release / upstream strict version tags：
...
看到新 tag 只代表需要 release review；在確認 upstream published stable release metadata 前，不要把它當成 available release，更不代表自動採用。

若要確認 / 同步 `origin` 的 official version-tag archive，依：
...
validate（目前 v1.18.32 contract-review baseline）：
