...
Bun version **不 hard-code 在 helper**。目前 checkout 若是 `v1.18.18`，helper 會從 `package.json` 取得 `bun@1.3.14`。
...
```bash
git show v1.18.22:package.json | cat
...
對目前 `sky/v1.18.18`，local patch 或 maintenance change 完成後至少：
...
這組 command 是 **目前 v1.18.18 已驗證的 baseline**，不是所有 future release 的永久 contract。
